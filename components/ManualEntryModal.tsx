...
const ManualEntryModal: React.FC<ManualEntryModalProps> = ({ onClose }) => {
  const [activeTab, setActiveTab] = useState<TabType>('EXPENSE');
  const [amount, setAmount] = useState('');
  const [selectedCategory, setSelectedCategory] = useState<string>('餐饮美食');
      ...
      <div className="w-full max-w-[520px] bg-white rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0">
          ...
          <div className="flex bg-background-light rounded-lg p-1 gap-1">
            <button 
              onClick={() => { setActiveTab('EXPENSE'); setSelectedCategory('餐饮美食'); }}
            ...
            <button 
              onClick={() => { setActiveTab('INCOME'); setSelectedCategory('工资收入'); }}
            ...
            <button 
              onClick={() => { setActiveTab('TRANSFER'); setSelectedCategory('存入'); }}
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center bg-background-light rounded-lg h-10 px-3 border border-transparent focus-within:border-primary/50 focus-within:bg-white transition-all">
                ...
                <input className="bg-transparent border-none text-sm text-text-main focus:ring-0 w-full p-0 placeholder:text-gray-400 outline-none" placeholder="写点什么..." type="text" />
              </div>
            </div>
          </div>
        ...
        <div className="p-6 pt-2 flex gap-3 shrink-0">
          ...
          <button onClick={() => { alert(`已保存: ${activeTab} ¥${amount} - ${selectedCategory}`); onClose(); }} className="flex-[2] h-12 rounded-xl bg-primary text-white font-bold text-base hover:bg-primary-hover transition-colors shadow-lg shadow-blue-200">保存</button>
        </div>
      </div>
    </div>
  );
